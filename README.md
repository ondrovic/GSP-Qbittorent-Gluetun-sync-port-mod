...
### Multi-Version Gluetun Support
The mod automatically supports both Gluetun v3 and v4+ by trying multiple API endpoints in order of preference:
- `/v1/portforward` (Gluetun v4+ - preferred)
- `/v1/openvpn/portforwarded` (Gluetun v3 - legacy)

This ensures compatibility across Gluetun versions without requiring configuration changes.

### Automatic Port Verification
The mod automatically verifies that the forwarded port is actually open and reachable from the internet using an external port checking service. This check runs:
...
```toml
[[roles]]
name = "ondrovic/GSP-Qbittorent-Gluetun-sync-port-mod"
...
With the following link for the mod `ghcr.io/ondrovic/gsp-qbittorent-gluetun-sync-port-mod:main`.
...
GSP_qbt_gtn_sync_port:
  ...
  environment:
      - DOCKER_MODS=ghcr.io/ondrovic/gsp-qbittorent-gluetun-sync-port-mod:main
...
There are 2 main issues with HTTPS :
 ...
 - Your certificate is trusted, but does not contain `localhost` (obviously) and so the connection is refused.
    ...
    For this one, you can check [Unspec7's guide](https://github.com/ondrovic/GSP-Qbittorent-Gluetun-sync-port-mod/issues/14) (Thanks to him).
